...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    if (!empty($_GET['action']) && $_GET['action'] == 'delete') {
        ...
        header("Location: one.php"); // Redirect back to the list page
        exit;
    }
}
?>
        ...
        <div class="row">
            ...
            foreach ($stmt as $row) {
                ...
                <div class="col-md-4 mb-4">
                    <div class="card">
                        <img src="<?= htmlspecialchars($row['picture']) ?>" class="card-img-top" alt="Employee Picture" style="width: 100%; height: 200px; object-fit: cover;">
                        <div class="card-body">
                            <h5 class="card-title"><?= htmlspecialchars($row['name']) ?></h5>
                            <p class="card-text">
                                <strong>Email:</strong> <?= htmlspecialchars($row['email']) ?><br>
                                <strong>Phone:</strong> <?= htmlspecialchars($row['phone']) ?><br>
                                <strong>Manager ID:</strong> <?= htmlspecialchars($row['manager_id']) ?>
                            </p>
                            <a href="edit.php?id=<?= htmlspecialchars($row['id']) ?>" class="btn btn-primary">Edit</a>
                            <a href="one.php?action=delete&id=<?= htmlspecialchars($row['id']) ?>" class="btn btn-danger">Delete</a>
                        </div>
                    </div>
                </div>
                <?php
